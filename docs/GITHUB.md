...
`macos` · `apple-silicon` · `appkit` · `swift` · `c` · `canon` · `bjc-85` ·
`is-12` · `scanner` · `printing` · `gutenprint` · `pappl` · `libusb`

## Images

The README leads with the Scan workspace, pairs Print with Copy and Device
with Dark Mode, and includes Settings in an expandable section. Six editable
SVGs, matching 2× PNGs and a contact sheet are in [images](images/README.md).
They are labelled as illustrative UI mockups. Actual runtime captures remain
